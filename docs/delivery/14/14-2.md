...
Add log subscription and fetching capabilities to the Railway client package (`api/internal/railway/`) to receive real-time logs via GraphQL subscriptions and query historical logs. This uses a hybrid approach: keeping the existing `machinebox/graphql` for queries and adding `hasura/go-graphql-client` for WebSocket-based subscriptions.
...
- **Modified**: `api/internal/railway/client.go` (add subscription endpoint field)
- **Modified**: `go.mod` (add hasura/go-graphql-client dependency)
- **Modified**: `go.sum` (dependency checksums)
