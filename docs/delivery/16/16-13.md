...
- `web/src/lib/api.ts` - Update fetchJSON with token parameter and 401 handling
- `web/src/hooks/useAuthenticatedFetch.ts` - Create authenticated fetch hook
- `web/src/hooks/useCurrentUser.ts` - Create current user hook
- `web/src/hooks/useRailway.ts` - Update to use authenticated fetch
- `web/src/store/wizard.ts` - Update to use authenticated fetch (if applicable)
