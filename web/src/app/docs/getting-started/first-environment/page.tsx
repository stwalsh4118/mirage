...
export default async function FirstEnvironmentPage() {
  const content = await getDocsContent("getting-started/first-environment.md");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center glass grain rounded-lg p-4 border border-border">
        ...
        <Button asChild>
          <Link href="/docs/getting-started/key-concepts">
            Learn Key Concepts <ArrowRight className="ml-2 h-4 w-4" />
          </Link>
        </Button>
      </div>
    </div>
